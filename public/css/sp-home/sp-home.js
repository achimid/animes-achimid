  ...
  return json.map(item => {
    return `
        <figure class="snip1577" style="width: ${width}; height: ${height};">
          ...
          <a href="#" target="_blank" onclick="searchSources('${item.anime.name}')"></a>          
        </figure>              
    `
  }).join('')
}
  ...
  return json.map(item => {
    ...
    <tr class="new">
        <td class="release-item">            
            ...
            <a href="${item.anime.url}" data-preview-image="${item.anime.image}">${item.title.split(' - ')[1].substring(0, 60)}</a>                      
        </td>                                                                            
