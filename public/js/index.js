...
const loadUsersViewsInfo = async () => {
  if (!isAdmin()) return

  fetch("/api/v1/user/users-views-info")
    .then(res => res.json())
    .then(info => {
        document.querySelector('#vInfo').innerHTML = `Users: ${info.users} Views: ${info.views}`
    })
}
...
const renderBell = (item) => {
  if (!getUser().animeToNotify.includes(item.anime._id)) {
    return `
      <a href="#" target="_blank" onclick="notifyAnime(this, '${item.anime._id}')"  class="cls-notify ${isAuthenticated() ? '' : 'cls-notify-not-logged'}">
    ...
    return `
    <a href="#" target="_blank" onclick="notifyAnimeCancel(this, '${item.anime._id}')"  class="cls-notify ${isAuthenticated() ? '' : 'cls-notify-not-logged'}">      
