function onLoad() {
    ...
    fetch('/api/v1/anime-info/show/' + id).then(res => res.json()).then(anime => {
        ...
        if (!getUser().animeToNotify.includes(id)) {
            ...
            <a href="#" target="_blank" onclick="notifyAnime('${anime._id}')" class="cls-notify" title="Gostaria de ser notificado quando houver um novo lançamento!">
            ...
            document.querySelector('.entry-header').innerHTML = `
            <a href="#" target="_blank" onclick="notifyAnimeCancel('${anime._id}')" class="cls-notify" title="Gostaria de ser não ser mais notificado!">
